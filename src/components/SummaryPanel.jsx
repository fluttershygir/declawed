...
import { FileCheck, AlertCircle, Calendar, ShieldCheck, AlertTriangle, ListChecks, Download, Lock, Mail, Building2, RotateCcw, ExternalLink, CheckCircle2, Share2, FileImage } from 'lucide-react';
                ...
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      className="mt-3 w-full rounded-xl border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.13] active:scale-[0.99] transition-all px-3.5 py-2.5 flex items-center gap-3 group disabled:opacity-60"
                    >
                      {/* Score badge */}
                      <div className={`w-8 h-8 rounded-full border-2 ${sb} flex items-center justify-center shrink-0`}>
                        {score !== null
                          ? <span className={`text-[11px] font-black tabular-nums leading-none ${sc}`}>{score}</span>
                          : <span className="text-[10px] text-zinc-600">–</span>}
                      </div>
                      <div className="flex-1 min-w-0 text-left">
                        <p className="text-[12px] font-semibold text-zinc-300 group-hover:text-white transition leading-tight">Save score card</p>
                        <p className="text-[10px] text-zinc-600 leading-tight mt-0.5">1080 × 1080 image · download or share</p>
                      </div>
                      <div className="shrink-0 w-6 h-6 rounded-md border border-white/[0.07] bg-white/[0.03] flex items-center justify-center group-hover:border-white/[0.12] transition">
                        {imgShareState === 'loading' ? (
                          <div className="w-3 h-3 border border-zinc-600 border-t-zinc-400 rounded-full animate-spin" />
                        ) : imgShareState === 'done' ? (
                          <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                        ) : (
                          <FileImage className="w-3 h-3 text-zinc-500 group-hover:text-zinc-300 transition" />
                        )}
                      </div>
                    </button>
                  );
